...
The Slack Marketplace is where teams discover and install apps that extend their workspace. Listing your app there puts it in front of millions of potential users. Getting from "working prototype" to "approved listing" requires deliberate preparation beyond writing functional code.
...
This guide is organized into ten posts, each focused on a stage of the marketplace journey. After this introduction, we start with planning: choosing interaction surfaces, designing for multi-workspace from day one, and scoping permissions. From there, we cover building for distribution: OAuth install flows, token management, persistent state, and hosting.

The middle sections cover what makes or breaks a marketplace listing in practice. We walk through manifest configuration, UX polish, testing strategies, and the security and privacy requirements that trip up many first-time submissions.

The final posts focus on the marketplace itself: preparing your listing, navigating the review process, and maintaining your app after launch. Each post is self-contained but builds on concepts from earlier sections.
...
The app uses four interaction surfaces: a global shortcut, an App Home tab, DM messages, and modals. It renders headers, sections, actions, context blocks, dividers, images, styled buttons, and confirm dialogs. Together these cover most of the Block Kit surface area. The game updates a single message in place via `chat.update` rather than posting new messages per choice. Modal form submissions feed user-provided text back into the narrative through template resolution.

The codebase separates concerns cleanly. Listeners are organized by type, game logic is isolated from Block Kit rendering, and story data lives in a declarative graph structure. The current codebase runs in Socket Mode for a single workspace. Throughout this guide, we'll highlight what it does well and what would need to change for marketplace distribution.

## Prerequisites

This guide assumes you already have a working Slack app in at least one workspace, even just a development workspace. You'll need a Slack developer account at [api.slack.com/apps](https://api.slack.com/apps) and your source under version control, since we'll be making structural changes.
